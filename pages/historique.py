...
# Resserre l'espace entre les lignes de la liste (uniquement celle-ci, pas le
# reste de la page) : Streamlit espace chaque widget de 16px par défaut (gap
# du conteneur flex stVerticalBlock), ce qui devient vite beaucoup une fois
# repliés sur plusieurs dizaines d'entrées. st.container(key=...) donne une
# classe CSS ("st-key-<key>") ciblable sur SON PROPRE conteneur, sans toucher
# à l'espacement des filtres/sections au-dessus.
st.markdown(
    """
    <style>
    .st-key-liste_historique {
        gap: 0.35rem;
    }
    </style>
    """,
...
with st.container(key="liste_historique"):
    for e in filtered:
        titre = (
